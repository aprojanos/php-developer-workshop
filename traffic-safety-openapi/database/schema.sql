...
CREATE INDEX IF NOT EXISTS ix_user_refresh_tokens_user ON user_refresh_tokens (user_id);
CREATE INDEX IF NOT EXISTS ix_user_refresh_tokens_expires_at ON user_refresh_tokens (expires_at);

CREATE TABLE IF NOT EXISTS user_access_tokens (
    id BIGSERIAL PRIMARY KEY,
    user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    token_id CHAR(64) NOT NULL UNIQUE,
    issued_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    expires_at TIMESTAMPTZ NOT NULL,
    revoked_at TIMESTAMPTZ
);

CREATE INDEX IF NOT EXISTS ix_user_access_tokens_user ON user_access_tokens (user_id);
CREATE INDEX IF NOT EXISTS ix_user_access_tokens_expires_at ON user_access_tokens (expires_at);
