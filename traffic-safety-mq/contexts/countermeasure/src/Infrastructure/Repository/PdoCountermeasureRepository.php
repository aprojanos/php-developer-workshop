...
namespace CountermeasureContext\Infrastructure\Repository;
...
final class PdoCountermeasureRepository implements CountermeasureRepositoryInterface
{
    ...
    public function all(): array
    {
        ...
        return array_map([$this, 'rowToCountermeasure'], $rows);
    }
    ...
    public function findById(int $id): ?Countermeasure
    {
        ...
        return $row !== false ? $this->rowToCountermeasure($row) : null;
    }
    ...
    {
        if ($countermeasure instanceof IntersectionCountermeasure) {
            return [
        ...
        if ($countermeasure instanceof RoadSegmentCountermeasure) {
            return [
                'road_classifications' => array_map(
                    static fn($classification) => $classification->value,
    ...
    {
        $values = array_map(static fn($type) => $type->value, $collisionTypes);
    ...
    {
        $values = array_map(static fn($severity) => $severity->value, $severities);
    ...
    /**
     * @param array<\SharedKernel\Enum\CollisionType> $requiredTypes
     */
    ...
    /**
     * @param array<\SharedKernel\Enum\InjurySeverity> $requiredSeverities
     */
